        ...
        for(let k in map){
            ...
            if(obj.type=="Array"){
                ...
                if(obj.ctor=="Integer"||obj.ctor=="Float"||obj.ctor=="Boolean"||obj.ctor=="String"){
                    ...
                    val = val?.map((item:any/**import("cc").EventHandler */)=>{return {node:item?.target?.uuid,comp:item?._componentId,handler:item?.handler,customEventData :val?.customEventData}})
                }else{
                    const cls = _xx.js.getClassByName(obj.ctor)
                    if(obj.ctor=="cc.Component"){
                        val = val?.map((a)=>{return a.uuid??a._uuid})
                ...
                if(val){
                    if(val instanceof _cc_().EventHandler){
                        val = {node:val?.target?.uuid,comp:val?._componentId,handler:val?.handler,customEventData :val?.customEventData}
                    }
                }else{
                    val = ""
                }
...
function _getAttrInfosOfComponentProrotype(clsPrototype){
    ...
    for(let k of props){
        ...
        if(Array.isArray(param.default)){
            if(param.ctor==null&&param.type?.name){
                param.ctor = param.type?.name
            }
            param.type = "Array"
        }else if(!param.type){
...
function isSubclass(childClass: any, parentClass: any): boolean {
